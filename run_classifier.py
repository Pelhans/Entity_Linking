...
flags.DEFINE_string(
    "serving_model_save_path", None, "The pb data output dir.  ")
...
class POIProcessor(DataProcessor):
  """Processor for the CoLA data set (GLUE version)."""
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      out_label = tf.argmax(logits, axis=-1, output_type=tf.int32)
      in_labels = tf.identity(label_ids)
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"result": out_label,
                       "label_ids": in_labels,
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        results_ids = prediction["result"]
        if i >= num_actual_predict_examples:
          break
#        output_line = "\t".join( [str(results_ids), str(labels_ids)]) + "\n"
        output_line = "\t".join( [str(labels_ids), str(results_ids)]) + "\n"
